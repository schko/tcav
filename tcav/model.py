...
class ModelWrapper(six.with_metaclass(ABCMeta, object)):
  ...
  def get_gradient(self, acts, y, bottleneck_name, example):
    ...
    """
    print('----get_gradient----')
    print('self.bottlenecks_gradients: ', self.bottlenecks_gradients)
    print('self.bottlenecks_gradients[bottleneck_name]: ', self.bottlenecks_gradients[bottleneck_name])
    ...
    print('----attempting to run session----')
...
class PublicImageModelWrapper(ImageModelWrapper):
  ...
  def get_bottleneck_tensors(scope, default_vars):
    ...
    if default_vars:
        ...
        op_type = 'concat/axis' #for mixed** layers
...
class CNNFCFRNetWrapper_public(PublicImageModelWrapper):
    ...
    def __init__(self, sess, model_saved_path, labels_path, default_vars):
      ...
      endpoints_v1 = dict(
        ...
        # logit='softmax2_pre_activation:0',
        prediction='activation_4/Sigmoid:0',
        # pre_avgpool='mixed5b:0',
        # logit_weight='softmax2_w:0',
        # logit_bias='softmax2_b:0',
    )
      self.sess = sess
      super(CNNFCFRNetWrapper_public, self).__init__(sess,
                                                  model_saved_path,
                                                  labels_path,
                                                  image_shape_v1,
                                                  endpoints_v1,
                                                  scope='v1', default_vars=default_vars)
      self.model_name = 'CNNFCFRNet_public'
...
class inceptionV3multiClassWrapper_public(PublicImageModelWrapper):
    ...
    def __init__(self, sess, model_saved_path, labels_path, default_vars):
      ...
      self.sess = sess
      super(inceptionV3multiClassWrapper_public, self).__init__(sess,
                                                  model_saved_path,
                                                  labels_path,
                                                  image_shape_v3,
                                                  endpoints_v1,
                                                  scope='inception_v3', default_vars=default_vars)
      self.model_name = 'inceptionV3multiClass_public'
